...
- **Product Type** (productType): `'RC'` (fixed discriminator)
...
- **Variant** (variant): String
  - Options:
    - `'standard_barrier_rc'` - Standard Barrier Reverse Convertible
...
### Conversion Terms
- **Conversion Ratio** (conversionRatio): Number
  - Example: 1.0
  - Must be > 0
  - Represents shares per unit of notional
  - Typically 1.0
...
### Reference Pricing (Optional - for pre-trade reports)
- **Reference Mode** (referenceMode): String
  - Options: `'today'`, `'custom'`
  - Default: `'today'` (uses current spot price)
- **Reference Date** (referenceDate): ISO Date String (YYYY-MM-DD)
  - Required only if referenceMode = 'custom'
